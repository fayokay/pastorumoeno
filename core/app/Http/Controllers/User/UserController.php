...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->password_reset==""){
         ...
         $data['wardscnt'] = Quote::where('verified', 'Yes')->distinct('ward')->count('id');
        $data['unitscnt'] = Quote::where('verified', 'Yes')->distinct('pu')->count('id');
        $data['lgacnt'] = Quote::where('verified', 'Yes')->distinct('lga')->count('id');
        ...
        if ($request->cpass) {
            if (Hash::check($request->cpass, $user->password)) {
                if ($request->npass == $request->cfpass) {
                    ...
                    $user->password_reset =  date('Y-m-d H:i:s');
                } else {
